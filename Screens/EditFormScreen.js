...
const EditFormScreen = ({route, navigation}) => {
  ...
  const {id, pengajuan, token} = route.params;

  const [dataPengajuan, setData] = useState(pengajuan);
    ...
    return (
      <View style={styles.containerInput} key={j}>
        ...
        <Datepicker
          ...
          date={date[i - 1] === date[i - 1] ? value : date[i - 1]}
          min={min}
          onSelect={nextDate => {
            // format date
          ...
          {layanan[id].form.map((data, i) => {
            ...
            if (data.type === 'disabled') {
              return inputDisabled(
                data.label,
                dataPengajuan[data.fill],
                data.fill,
                i,
              );
            } else if (data.type === 'input') {
              return input(data.label, dataPengajuan[data.fill], data.fill, i);
